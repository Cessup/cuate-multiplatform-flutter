...
class ForgotPasswordNotifier extends StateNotifier<AsyncValue<AuthReset>> {
  ...
  ForgotPasswordNotifier(
      {required this.resetPassword, required this.resetPasswordConfirm})
    ...
    try {
      if (email.isEmpty) {
        state = AsyncValue.error(
            Exception('Email can not be empty'), StackTrace as StackTrace);
        return;
      }
  ...
  Future<void> codeConfirm(
    ...
    try {
      ...
      final resetPasswordConfirmParams =
          ResetPasswordConfirmParams(email, code, newPassword);
